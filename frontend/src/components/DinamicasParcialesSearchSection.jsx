...
function DinamicasParcialesSearchSection() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const handle = await window.showSaveFilePicker({
        suggestedName: fileName || 'dinamicas_parciales.dpf',
        types: [{ description: 'Dinámicas Parciales File', accept: { 'application/json': ['.dpf'] } }]
      });
  ...
  const loadFromData = (data) => {
    ...
    setIsStructureCreated(true);
    setFileName(data.fileName || 'archivo.dpf');
    setHasUnsavedChanges(false);
    setHistory([]);
    setHistoryIndex(-1);
  ...
  useEffect(() => {
    ...
    window.dinamicasParcialesCheckUnsavedChanges = checkUnsavedChanges;
    return () => {
